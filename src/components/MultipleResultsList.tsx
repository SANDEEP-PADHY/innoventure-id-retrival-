...
export const MultipleResultsList: React.FC<MultipleResultsListProps> = ({
  ...
  return (
    <div
      ...
      className="bg-white border border-slate-200/90 rounded-2xl shadow-card overflow-hidden"
    >
      {/* Header */}
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs sm:text-sm text-slate-500 mt-0.5">
              Found <span className="font-bold text-slate-800">{students.length} students</span> matching &ldquo;<span className="text-brand-800 font-semibold">{query}</span>&rdquo;. Please select a record to view complete details:
            </p>
          </div>
        </div>
      </div>
          ...
          return (
            ...
            >
              <div className="flex items-center gap-3.5">
                <div className="w-11 h-11 rounded-xl bg-slate-100 text-slate-700 font-bold flex items-center justify-center text-sm group-hover:bg-brand-800 group-hover:text-white transition-colors flex-shrink-0">
                  {student.name
                    ? student.name
                        .split(' ')
                        .filter(Boolean)
                ...
                <div>
                  ...
                  <div className="flex flex-wrap items-center gap-3 mt-1 text-xs text-slate-500">
                    <span className="flex items-center gap-1 font-mono text-slate-700 font-semibold bg-slate-50 px-1.5 py-0.5 rounded border border-slate-200">
              ...
              <div className="flex items-center justify-end sm:justify-start">
                <button
                  ...
                  className="px-4 py-2 bg-white group-hover:bg-brand-800 text-slate-700 group-hover:text-white border border-slate-200 group-hover:border-brand-800 rounded-lg text-xs font-bold flex items-center gap-1.5 transition-all shadow-sm"
                >
